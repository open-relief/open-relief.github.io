...
import { transfers, fundRequests, persistFundRequests, setCachedOutgoingToken } from '../src/store.js'
import {
  createClient,
...
async function handleTransferCallback(req, res) {
  ...
  try {
    ...
    // Cache token for future payouts — avoids re-consent (Option 3: token reuse)
    if (transfer.type === 'admin-payout') {
      const expiresIn = Number(finalizedGrant?.access_token?.expires_in || 0)
      const safeSeconds = expiresIn > 60 ? expiresIn - 60 : Math.max(expiresIn, 3540)
      const expiresAt = new Date(Date.now() + safeSeconds * 1000).toISOString()
      setCachedOutgoingToken({
        value: outgoingAccessToken,
        expiresAt,
        resourceServer: transfer.senderResourceServer,
        walletAddress: transfer.senderWalletAddress
      })
      console.log('[token-cache] Token cached after consent. Expires:', expiresAt)
    }
